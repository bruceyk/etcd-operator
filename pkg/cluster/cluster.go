...
func (c *Cluster) startSeedMember() error {
	...
	_, err := c.eventsCli.Create(context.Background(), k8sutil.NewMemberAddEvent(m.Name, c.cluster), metav1.CreateOptions{})
	if err != nil {
...
func (c *Cluster) removePod(name string) error {
	...
	err := c.config.KubeCli.CoreV1().Pods(ns).Delete(context.Background(), name, *opts)
	if err != nil {
...
func (c *Cluster) reportFailedStatus() {
	...
	f := func() (bool, error) {
		...
		cl, err := c.config.EtcdCRCli.EtcdV1beta2().EtcdClusters(c.cluster.Namespace).
			Get(context.Background(), c.cluster.Name, metav1.GetOptions{})
		if err != nil {
