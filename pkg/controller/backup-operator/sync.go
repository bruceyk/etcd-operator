...
func (b *Backup) addFinalizerOfPeriodicBackupIfNeed(eb *api.EtcdBackup) (*api.EtcdBackup, error) {
	...
	if !containsString(metadata.GetFinalizers(), "backup-operator-periodic") {
		...
		_, err := b.backupCRCli.EtcdV1beta2().EtcdBackups(b.namespace).Update(context.Background(), ebNew.(*api.EtcdBackup), metav1.UpdateOptions{})
		if err != nil {
			return eb, err
		}
	...
	for {
		...
		case <-t.C:
			...
			for i := 1; i < retryLimit+1; i++ {
				latestEb, err = b.backupCRCli.EtcdV1beta2().EtcdBackups(b.namespace).Get(ctx, eb.Name, metav1.GetOptions{})
				if err != nil {
...
func (b *Backup) reportBackupStatus(bs *api.BackupStatus, berr error, eb *api.EtcdBackup) {
	...
	_, err := b.backupCRCli.EtcdV1beta2().EtcdBackups(b.namespace).Update(context.Background(), eb, metav1.UpdateOptions{})
	if err != nil {
