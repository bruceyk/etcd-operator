...
func (r *Restore) reportStatus(rerr error, er *api.EtcdRestore) {
	...
	_, err := r.etcdCRCli.EtcdV1beta2().EtcdRestores(r.namespace).Update(context.Background(), er, metav1.UpdateOptions{})
	if err != nil {
...
func (r *Restore) createSeedMember(ec *api.EtcdCluster, svcAddr, clusterName string, owner metav1.OwnerReference) error {
	...
	_, err := r.kubecli.CoreV1().Pods(r.namespace).Create(context.Background(), pod, metav1.CreateOptions{})
	return err
}
...
func (r *Restore) deleteClusterResourcesCompletely(clusterName string) error {
	ctx := context.Background()
	// Delete etcd pods
	err := r.kubecli.CoreV1().Pods(r.namespace).Delete(ctx, clusterName, metav1.DeleteOptions{})
	if err != nil && !k8sutil.IsKubernetesResourceNotFoundError(err) {
		return fmt.Errorf("failed to delete cluster pods: %v", err)
	}

	err = r.kubecli.CoreV1().Services(r.namespace).Delete(ctx, clusterName, metav1.DeleteOptions{})
	if err != nil && !k8sutil.IsKubernetesResourceNotFoundError(err) {
